...
///
/// 注意：这是 best-effort 启发式检测，存在已知局限：
/// - 括号计数不区分字符串内外的括号，可能误判含 `{}` 的字符串值
/// - 适用于截断检测场景，误报不影响功能正确性（仅用于日志诊断）
...
///
/// 注意：简单按逗号 split，JSON 值中的逗号（字符串内、嵌套结构）可能导致
/// 字段提取不准确。仅用于诊断日志，不影响功能逻辑。
...
fn detect_content_truncation(
    ...
    // 启发式：content 在 raw_input 中占比异常低（排除 schema 元数据干扰）
    // 仅当 raw_input 足够大且 content 占比低于 5% 时才判定为截断
    if raw_input.len() > 2000 && (content.len() as f64 / raw_input.len() as f64) < 0.05 {
        return Some(
